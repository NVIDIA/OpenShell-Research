...
Use `on_error: fail_closed` when a Privacy Guard RPC failure must stop the
...
| `config.engine` | Yes | Engine discriminator registered by the running Privacy Guard process |

Stage names must be unique when supplied. Privacy Guard derives names such as
