...
`RegexEngine` is the built-in Egress Gate engine. It detects every configured
regular-expression match and can replace a deterministic non-overlapping subset
with a constrained template.

Egress Gate provides the catalog schema and execution bounds. It does not
...
- is relative to Egress Gate's working directory
...
input. Egress Gate enforces a shared timeout, but a slow pattern can consume
...
[`projects/egress-gate/examples/regex-engine`](https://github.com/NVIDIA/OpenShell-Research/tree/main/projects/egress-gate/examples/regex-engine).
