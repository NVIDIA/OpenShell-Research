---
title: Egress Gate
description: Overview and quickstart for running Egress Gate with OpenShell.
agent_markdown: true
---
...
Egress Gate does not intercept prompts, tool output, transcripts, or session
history before a harness writes them to disk. Those files may contain raw
sensitive values even when Egress Gate later replaces or blocks the network
...
Egress Gate processes the complete request body as UTF-8 text. It does not
...
Egress Gate. Egress Gate returns an allow, replacement, or deny decision
before OpenShell attaches credentials and sends an allowed request to the
provider.](assets/diagrams/request-path.svg)

Egress Gate exposes OpenShell's `SupervisorMiddleware` gRPC service. It
...
Egress Gate is tested with the version recorded in the
[middleware manifest](https://github.com/NVIDIA/OpenShell-Research/blob/main/projects/egress-gate/.openshell-middleware-manifest.json).
...
See [Run and operate Egress Gate](operations.md) for cleanup, gateway
...
Egress Gate startup selects which engine implementations and operational
...
- [Configure policies](configuration.md): stages, actions, catalogs, and policy
  recipes.
- [Run and operate Egress Gate](operations.md): CLI, gateway registration,
