...
description: Egress Gate request, processing, Regex, concurrency, logging, and failure reference.
agent_markdown: true
---

# Limits and failure behavior

Egress Gate enforces bounds at the transport, processor, engine, and result
boundaries. Limit failures are atomic: they return no partial replacement text
or partial findings.

Package-wide values are defined in
`projects/egress-gate/src/egress_gate/constants.py`.
...
```bash
egress-gate serve --timeout-seconds 4
```

OpenShell's middleware timeout must be longer than the Egress Gate processing
...
Egress Gate returns `egress_gate_limit_exceeded` with no partial findings.
...
| Detection with `block` | Deny | `egress_gate_blocked` |
| Shared timeout expires | Deny | `egress_gate_limit_exceeded` |
| Processing or result limit exceeded | Deny | `egress_gate_limit_exceeded` |
...
When a request returns `egress_gate_limit_exceeded`:

1. inspect Egress Gate logs for the content-safe limit kind
...
Production errors use stable `EgressGateError` codes. Error responses and
logs do not expose raw Pydantic, regex backend, protobuf, engine, or collaborator
exception messages.

Expected custom-engine failures must be translated into Egress Gate's
...
Egress Gate retains one active validated policy and its configured engines
...
![Scatter plot of Egress Gate latency against prompt tokens on logarithmic axes. Point color maps the number of entities detected from 3 to 429. Ninety-six service measurements follow one fitted line. A dashed line marks the nominal one-million-token context threshold.](../assets/analysis/egress-gate-latency-vs-prompt-size.svg)
...
[`projects/egress-gate/analysis`](https://github.com/NVIDIA/OpenShell-Research/tree/main/projects/egress-gate/analysis)
directory.
...
5. run `make check` from `projects/egress-gate`
