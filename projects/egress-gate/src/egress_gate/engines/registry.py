...
class EngineRegistry:
    ...
    def __init__(self, *, include_builtin_engines: bool = False) -> None:
        ...
        self._config_adapter: TypeAdapter[EgressGateConfig[EngineConfig]] | None = None
    ...
    def validate_config(self, values: object) -> EgressGateConfig[EngineConfig]:
        """Purely parse and validate an expanded Egress Gate configuration."""
    ...
    def _require_config_adapter(
        self,
    ) -> TypeAdapter[EgressGateConfig[EngineConfig]]:
...
def _build_egress_gate_config_type(
...
) -> type[EgressGateConfig[EngineConfig]]:
    if not config_types:
    ...
    config_type = EgressGateConfig.__class_getitem__(
        registered_config  # ty: ignore[invalid-argument-type]
    )
    if not isinstance(config_type, type) or not issubclass(
        config_type, EgressGateConfig
    ):
