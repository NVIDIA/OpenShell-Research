...
from egress_gate.config import EgressGateConfig, PolicyAction
...
class RequestProcessor:
    ...
    def process(self, text: str) -> RequestProcessingResult:
        ...
        if self._log_request_content:
            _LOGGER.debug("egress_gate_text_input text=%r", input_text)
        ...
        try:
            for source, engine in self._engines:
                _LOGGER.debug(
                    "egress_gate_stage_run source=%s strategy=%s",
                    source,
                    strategy.value,
                )
                result = engine.run(
            ...
            raise EgressGateError(ErrorCode.ENGINE_OUTPUT_INVALID) from None
        except EntityProcessingError:
            raise EgressGateError(ErrorCode.ENGINE_EXECUTION_FAILED) from None
        ...
        if self._log_request_content:
            _LOGGER.debug("egress_gate_text_output text=%r", current_text)
        return RequestProcessingResult(
