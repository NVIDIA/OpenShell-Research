...
def test_every_action_prints_without_secrets_or_side_effects(tmp_path: Path) -> None:
    ...
    for action in (
        ...
        result = subprocess.run(
            ...
            capture_output=True,
            text=True,
            env=os.environ | {"PI_MODEL_API_KEY": "PRIVATE_TEST_VALUE"},
        )
    ...
    assert "v0.0.116" in output
    assert "sha256sum --check" in output
...
def test_preparation_uses_upstream_profiles_and_excludes_private_material(
    ...
    gateway = tomllib.loads((state / "gateway.toml").read_text())
    registration = gateway["openshell"]["supervisor"]["middleware"][0]
    assert registration["grpc_endpoint"] == "https://192.0.2.10:50051"
