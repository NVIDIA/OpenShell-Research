...
from egress_gate.config import PolicyAction
from egress_gate.constants import MAX_BODY_BYTES
from egress_gate.engines import RegexEngine
...
from egress_gate.request_processor import RequestDecision, RequestProcessor
...
def test_block_is_a_processor_disposition_not_an_engine_strategy() -> None:
    ...
    assert result.replacement_text is None
    assert result.reason_code == "egress_gate_blocked"
    assert tuple(item.entity for item in result.detection_summaries) == ("person",)
...
def test_timeout_returns_the_bounded_limit_deny(
    ...
    with caplog.at_level(logging.INFO, logger="egress_gate.request_processor"):
        result = _processor(PolicyAction.DETECT).process("Hello Alice")

    assert result.decision is RequestDecision.DENY
    assert result.reason_code == "egress_gate_limit_exceeded"
    assert "egress_gate_processing_limit kind=timeout" in caplog.text
