...
def test_configure_logging_emits_consistent_package_logs() -> None:
    ...
    logging.getLogger("egress_gate.service").info("server_started")
    logging.getLogger("egress_gate.service").debug("hidden_detail")
...
def test_configure_logging_colors_interactive_output() -> None:
    ...
    assert "\033[36megress_gate.service\033[0m" in output
...
def test_configure_logging_can_disable_terminal_colors() -> None:
    ...
    logging.getLogger("egress_gate.service").error("startup_failed")
...
def test_configure_logging_accepts_native_log_levels() -> None:
    ...
    logging.getLogger("egress_gate.request_processor").debug("processing_diagnostic")
...
def test_configure_logging_replaces_its_previous_handler() -> None:
    ...
    logging.getLogger("egress_gate").info("configured_once")
...
def test_reset_logging_restores_application_logging() -> None:
    package_logger = logging.getLogger("egress_gate")
...
_SOURCE_ROOT = Path(__file__).parents[1] / "src" / "egress_gate"
