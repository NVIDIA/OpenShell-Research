...
from egress_gate.engines import RegexPatternCatalog
...
def test_regex_example_runs_through_the_middleware_boundary(
    ...
    values = yaml.safe_load((EXAMPLE_DIRECTORY / "egress-gate-config.yaml").read_text())
...
def test_builtin_registry_drives_documented_cli_discovery_and_schema() -> None:
    command = str(Path(sys.executable).with_name("egress-gate"))
...
def test_regex_walkthrough_uses_current_policy_and_gateway_schema() -> None:
    ...
    middleware_config = policy["network_middlewares"]["egress_gate_replace"]
    assert middleware_config["middleware"] == "egress-gate-regex"
    ...
    assert "uv run --locked egress-gate serve --listen 0.0.0.0:50051" in readme
    ...
    assert "--name egress-gate-regex" in readme
