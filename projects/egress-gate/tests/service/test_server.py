"""Programmatic Egress Gate server lifecycle tests."""
...
from egress_gate.errors import EgressGateError, EngineRegistryError, ErrorCode
...
def test_programmatic_server_runs_with_injected_registry_and_default_address(
    ...
    served: list[tuple[EgressGateServer, str]] = []

    async def record_serve(self: EgressGateServer, listen: str) -> None:
        served.append((self, listen))
        await self._middleware.close()

    monkeypatch.setattr(EgressGateServer, "serve_async", record_serve)
...
def test_programmatic_server_requires_an_explicit_finalized_registry() -> None:
    with pytest.raises(EngineRegistryError, match="finalized"):
        EgressGateServer(EngineRegistry())
...
def test_synchronous_server_exits_cleanly_after_keyboard_interrupt(
    ...
    async def interrupt(self: EgressGateServer, listen: str) -> None:
        del self, listen
        raise KeyboardInterrupt

    monkeypatch.setattr(EgressGateServer, "serve_async", interrupt)
...
def test_engine_import_does_not_load_the_server_transport() -> None:
    probe = (
        "import sys; "
        "import egress_gate.engines; "
        "assert 'egress_gate.service' not in sys.modules; "
...
async def test_serve_async_starts_waits_and_closes_on_normal_termination(
    ...
    with caplog.at_level(logging.INFO, logger="egress_gate.service.server"):
    ...
    assert fake_server.stop_graces == [0]
    assert closed == [server._middleware]
    assert "egress_gate_server_bound listen='127.0.0.1:50053'" in caplog.text
...
def _middleware() -> EgressGateMiddleware:
    return EgressGateMiddleware(create_builtin_registry())
