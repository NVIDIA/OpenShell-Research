"""Egress Gate command-line application tests."""
...
from egress_gate import cli as cli_module
from egress_gate.cli import app
...
from egress_gate.logging import reset_logging
...
def test_cli_add_gateway_registration_updates_the_default_xdg_config(
    ...
    assert "Registered egress-gate at http://192.168.1.20:50051" in output
    assert "start Egress Gate, then restart the OpenShell gateway" in output
...
def test_cli_remove_gateway_registration_removes_the_named_registration(
    ...
    assert result.exit_code == 0
    output = _plain_output(result)
    assert f"Removed egress-gate-regex from {path}" in output
    assert "restart the OpenShell gateway" in output
    assert "egress-gate-regex" not in path.read_text()
...
def test_cli_remove_gateway_registration_reports_absent_name(
    ...
    assert result.exit_code == 0
    assert f"No registration named egress-gate-regex found in {path}" in _plain_output(
        result
    )
...
def test_console_script_targets_the_cli_module() -> None:
    console_script = next(
        ...
        if entry_point.name == "egress-gate"
    )

    assert console_script.value == "egress_gate.cli:app"
...
def test_cli_serve_adapts_operational_options_to_the_programmatic_server(
    ...
    def record_serve_sync(self: EgressGateServer, listen: str) -> None:
        calls.append(
            (
                listen,
    ...
    monkeypatch.setattr(EgressGateServer, "serve_sync", record_serve_sync)
    ...
    assert "egress_gate_request_content_logging_enabled" in _plain_output(result)


def test_cli_serve_prints_cataloged_startup_errors_without_a_traceback(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    def fail_safely(_: EgressGateServer, listen: str) -> None:
        del listen
        raise EgressGateError(ErrorCode.SERVER_BIND_FAILED)

    monkeypatch.setattr(EgressGateServer, "serve_sync", fail_safely)
