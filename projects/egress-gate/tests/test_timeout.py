...
@pytest.mark.parametrize(
    "seconds",
    [True, 0, -1, 0.001, 1.0001, float("inf"), 30],
)
def test_timeout_duration_is_strict_positive_and_bounded(
    ...
    with pytest.raises(
        ValueError,
        match="between 10ms and 29s, using whole milliseconds",
    ):
...
@pytest.mark.parametrize(
    ("seconds", "duration"),
    [(0.01, "10ms"), (1.0, "1s"), (4.5, "4500ms"), (29.0, "29s")],
)
...
@pytest.mark.parametrize(
    ("duration", "seconds"),
    [("10ms", 0.01), ("500ms", 0.5), ("1s", 1.0), ("29s", 29.0)],
)
...
@pytest.mark.parametrize("duration", ["", "1", "1.5s", "0s", "30s", "1m"])
