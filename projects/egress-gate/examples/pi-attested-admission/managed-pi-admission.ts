...
// A byte encoded as a JSON array item can occupy four characters including its comma.
...
export function createOpenShellContextAdmission(
	...
	async function requestAdmission(
		...
		if (encoded.byteLength > MAX_BRIDGE_RESPONSE_BYTES) throw new Error("OpenShell admission response is too large");
...
function denied(reasonCode?: string): { action: "deny"; reason: string } {
	return {
		action: "deny",
		reason: reasonCode ? `OpenShell denied this context addition (${reasonCode})` : "OpenShell denied this context addition",
	};
}
