 ...
 *
 * Configuration arrives as base64 JSON in LAB_DRIVER_CONFIG_B64. Every line it
 * writes to stdout is one LabEvent (see src/events.ts).
 */
...
async function main(): Promise<void> {
  const encoded = process.env.LAB_DRIVER_CONFIG_B64
  if (!encoded) throw new Error('LAB_DRIVER_CONFIG_B64 is required')
  const config = decodeDriverConfig(encoded)
  const runtime: Runtime = selectRuntime(config.runtime)
  const workdir = await mkdtemp(path.join(tmpdir(), 'lab-driver-'))
  ...
  const rotate = (reason: string): void => {
    ...
    emit({ type: 'lab.rotation', reason, fromEpoch: epoch, toEpoch: epoch + 1, rotation: rotations + 1, retainedCharacters: checkpoint.length, checkpoint })
    rotations += 1
    epoch += 1
    threadId = undefined
    successfulTurns = 0
  ...
  while (Date.now() < config.deadlineMs) {
    ...
    const context = {
      ...
      turn,
      signal,
      emit: (event: LabEvent) => { tally.record(event); emit(event) },
    }
    ...
    if (result.transient) {
      ...
      emit({ type: 'lab.backoff', reason: result.timedOut ? 'request_timeout' : 'transient_error', attempt: consecutiveFailures, delayMs: wait })
...
main().catch((error) => {
  emit({ type: 'lab.error', message: error instanceof Error ? error.message : String(error) })
  process.exitCode = 1
})
