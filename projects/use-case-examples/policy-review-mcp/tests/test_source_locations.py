...
def test_supported_groups_have_parser_derived_locations() -> None:
    policy = parse_policy((FIXTURES / "candidate-broad.yaml").read_text())
    network = next(group for group in policy.groups if group.kind == "github_rest")
    assert network.locations[0].pointer == "/network_policies/github/endpoints/0"
