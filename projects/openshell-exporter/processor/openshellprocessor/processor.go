...
func (p *processorImpl) processLogs(ctx context.Context, logs plog.Logs) (plog.Logs, error) {
	for resourceIndex := 0; resourceIndex < logs.ResourceLogs().Len(); resourceIndex++ {
		...
		p.redactAttributes(resourceLogs.Resource().Attributes())
		for scopeIndex := 0; scopeIndex < resourceLogs.ScopeLogs().Len(); scopeIndex++ {
			p.redactAttributes(resourceLogs.ScopeLogs().At(scopeIndex).Scope().Attributes())
...
func (p *processorImpl) normalize(ctx context.Context, record plog.LogRecord) {
	...
	// Identity is already fixed. Sanitize original attributes and any derived
	// strings that exporters serialize outside the redacted source copy.
	p.redactAttributes(record.Attributes())
	p.redactValue(record.Body())
}
