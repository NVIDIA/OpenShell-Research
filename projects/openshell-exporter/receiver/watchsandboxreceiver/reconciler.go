...
func (r *watchReceiver) policyReconciliationWorker(ctx context.Context) {
	...
	for {
		...
		case request := <-r.reconcileCh:
			r.recordPolicyQueueState(ctx)
			r.mu.Lock()
			_, exists := r.sandboxes[request.Sandbox.ID]
			r.mu.Unlock()
			if exists {
				r.reconcileSandbox(ctx, request)
			}
			// Keep the pending marker through completion: one sandbox can have
			// only one queued or active read, without retaining per-sandbox locks.
