...
description: Privacy Guard gRPC methods, protobuf translation, active policy management, and worker scheduling.
agent_markdown: true
---
...
to Privacy Guard's protobuf-free domain model. No other handwritten package
...
```text
validated PrivacyGuardConfig + configured RequestProcessor
```
...
| Privacy Guard error kind | gRPC status |
| --- | --- |
...
- OpenShell follows the explicit deny after `privacy_guard_blocked` or
  `privacy_guard_limit_exceeded`.

## Server API

`PrivacyGuardServer` owns the finalized registry, middleware adapter, gRPC
...
See [Run and operate Privacy Guard](../operations.md) for CLI startup and
