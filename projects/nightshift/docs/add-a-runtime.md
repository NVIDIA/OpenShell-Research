...
| `codex` | OpenAI Codex CLI under `codex exec --json`, mapped onto the common events. | `OPENAI_API_KEY` and the pinned Codex image (`npm run image:codex`), selected automatically. The base image's older Codex has an incompatible model-catalog schema. |
| `claude-code` | Anthropic Claude Code CLI under `claude -p --output-format stream-json`, mapped onto the common events. | A key for an Anthropic-format endpoint: `api.anthropic.com`, or any gateway that serves `/v1/messages`, set through `NIGHTSHIFT_MODEL_BASE_URL` and `NIGHTSHIFT_MODEL_API_KEY`. Runs on the base image; pin a version for evidence runs. |
...
- **Transient failures.** A result with `ok: false` and `transient: true`
  ...
  thread. Every wait is recorded as `driver.backoff`.
- **Rotation.** The driver starts a fresh thread with a bounded checkpoint of
  ...
  each one is recorded as `driver.rotation` with the exact checkpoint text.
...
- **Refusals.** A result with `refusal: true` records `driver.refusal`, strips the
...
- **Fatal failures.** A result with `ok: false` that is neither transient,
  ...
  run with `driver.error` and the given exit code.
...
| `driver.runtime` | Once from `setup`, naming the runtime, version, and model. |
| `driver.error` | A runtime-level error message worth preserving. |

The driver emits `turn.started`, `driver.backoff`, and `driver.rotation` itself.
Unparseable stdout lines are preserved by the host as `host.unparsed`.
