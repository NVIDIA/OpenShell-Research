...
| --- | --- | --- |
| `auto-approve` | Approves everything. | The zero-model fixture for `hello-canary`, and the ceiling condition for any experiment: what enforcement alone stops when oversight offers no resistance. |
| `reject-all` | Rejects everything. | The control condition: the policy never changes. |
| `model-reviewer` | A separate model with no tools decides against an invariant supplied by the experiment. | The AI reviewer in `github-policy-review`. |
...
export type ReviewerFactory = (options: { runDir: string; facts: Record<string, unknown> }) => Reviewer
...
directory and the experiment's instance facts. That is where per-run state such
...
A run with reviewer failures that never reached `done` is invalid.
...
- The factory receives the experiment folder's rendered `reviewer.md` as its
  instructions and the instance facts, so the invariant is owned by the
  experiment, not the reviewer. It fails fast when the folder has no `reviewer.md`.
- The model endpoint, name, reasoning level, and key come from the
  `NIGHTSHIFT_REVIEWER_*` variables, independent of the agent model's settings.
...
Select it with `--reviewer mine`, or make it an experiment's default in
`experiment.json`.
