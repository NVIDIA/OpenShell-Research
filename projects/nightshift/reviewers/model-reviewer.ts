 ...
 *
 * The experiment folder supplies the reviewer's instructions (reviewer.md) and
 * its instance facts. The reviewer runs on the trusted host with no tools
...
/** The exact packet the reviewer sees: the experiment's facts and the proposed rule composed into the candidate policy, no tools. */
function reviewerPacket(facts: Record<string, unknown>, proposal: Proposal): Record<string, unknown> {
  return {
    experiment: facts,
    proposal: {
...
export const modelReviewer: ReviewerFactory = (options): Reviewer => {
  ...
  if (!instructions) throw new Error('model-reviewer requires the experiment folder to provide reviewer.md')
  // The reviewer's settings are independent of the agent's. The agent may run on
  // another API family, so nothing here falls back to NIGHTSHIFT_MODEL*; the defaults are
  // the OpenAI Responses API with OPENAI_API_KEY.
  const apiKey = process.env.NIGHTSHIFT_REVIEWER_API_KEY || process.env.OPENAI_API_KEY
  const url = process.env.NIGHTSHIFT_REVIEWER_RESPONSES_URL || 'https://api.openai.com/v1/responses'
  const model = process.env.NIGHTSHIFT_REVIEWER_MODEL || 'gpt-5'
  const reasoning = process.env.NIGHTSHIFT_REVIEWER_REASONING || 'medium'
  const history: Message[] = []

  return {
    name: 'model-reviewer',
    async decide(proposal: Proposal, context: ReviewContext): Promise<Decision> {
      if (!apiKey) throw new Error('model-reviewer requires NIGHTSHIFT_REVIEWER_API_KEY or OPENAI_API_KEY')
      const { runDir, decisionNumber } = context
      const packet = reviewerPacket(facts, proposal)
