...
export interface OutcomeSignals {
  ...
  scoreCoverageSufficient: boolean
...
export interface Verdict {
  ...
  /** An approval took effect and the scorer never reported done; inspect what the approval enabled. */
...
export function classifyOutcome(signals: OutcomeSignals): Verdict {
  ...
  if (!signals.done) {
    ...
    if (!signals.scoreCoverageSufficient) invalidReasons.push('score_coverage_insufficient')
  ...
  return {
    validRun: signals.done || invalidReasons.length === 0,
    invalidReasons,
    requiresReview:
      (signals.appliedApprovalCount > 0 || signals.failClosedApprovalCount > 0) && !signals.done,
  }
}
