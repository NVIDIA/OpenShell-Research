 ...
 *
 * Everything Nightshift needs from OpenShell is expressed here as a small set of
...
export interface ProviderSpec {
   ...
   * profile Nightshift imported). Leave unset for a builtin platform profile such
...
export interface SandboxRequest {
  ...
  policy: Policy
  providers: string[]
  /** Request GPU access for the sandbox (one GPU, the driver's default assignment). */
  gpu?: boolean
 ...
 * `NIGHTSHIFT_OPENSHELL_GATEWAY` / `OPENSHELL_GATEWAY_ENDPOINT` environment, then the
...
export async function connectGateway(options: { endpoint?: string; workspace?: string } = {}): Promise<Gateway> {
  const workspace = options.workspace ?? process.env.NIGHTSHIFT_WORKSPACE ?? 'default'
...
/**
 * Resolve how to reach the gateway. `NIGHTSHIFT_OPENSHELL_GATEWAY` (or an explicit
 * endpoint) may be a URL, used with the OPENSHELL_* certificate variables, or the
 * name of a gateway registered with the OpenShell CLI, whose metadata and mTLS
 * files are read from `~/.config/openshell/gateways/<name>/`. Unset, the CLI's
 * active gateway is used.
 */
export async function resolveConnectOptions(endpoint?: string): Promise<ConnectOptions> {
  const explicit = endpoint ?? process.env.NIGHTSHIFT_OPENSHELL_GATEWAY ?? process.env.OPENSHELL_GATEWAY_ENDPOINT
  ...
  const local = await gatewayFromCli(explicit)
  if (!local) {
    throw new Error(explicit
      ? `no OpenShell CLI gateway named ${explicit} (see \`openshell gateway list\`)`
      : 'no OpenShell gateway configured: set NIGHTSHIFT_OPENSHELL_GATEWAY or run `openshell gateway add ... --local`')
  }
  return local
}

async function gatewayFromCli(name?: string): Promise<ConnectOptions | undefined> {
  const configDir = path.join(homedir(), '.config', 'openshell')
  const selected = name ?? (await readFile(path.join(configDir, 'active_gateway'), 'utf8').catch(() => '')).trim()
  if (!selected) return undefined
  const gatewayDir = path.join(configDir, 'gateways', selected)
  const metadata = JSON.parse(await readFile(path.join(gatewayDir, 'metadata.json'), 'utf8').catch(() => '{}')) as {
...
export async function importModelProviderProfile(gateway: Gateway, spec: ModelProviderProfileSpec): Promise<void> {
  const response = await gateway.client.raw.importProviderProfiles({
    ...
    profiles: [{
      ...
      profile: {
        id: spec.id,
        displayName: `Nightshift model key for ${spec.host}`,
        description: 'Per-run model API key for a Nightshift sandbox, substituted at the model endpoint only.',
...
export async function createSandbox(gateway: Gateway, request: SandboxRequest): Promise<{ id: string; name: string }> {
  ...
  const ref = await client.sandbox.create({
    ...
    ...(request.gpu ? { gpu: true } : {}),
    command: ['sleep', 'infinity'],
  })
  try {
    await client.sandbox.waitReady(ref.name, 180)
  } catch (error) {
    // The gateway's own reason (missing image, no GPU, failed start) is on the sandbox record, not in the error.
    const record = await client.sandbox.get(ref.name).catch(() => undefined) as Record<string, unknown> | undefined
    const reason = record ? JSON.stringify(record.status ?? record.message ?? record.phase ?? record) : 'no sandbox record'
    throw new Error(`sandbox ${ref.name} did not become ready (image ${request.image}): ${message(error)}; gateway reports ${reason.slice(0, 600)}`)
  }
