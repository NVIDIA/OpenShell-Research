...
if (process.argv.includes("--tui")) {
  ...
  const interactive = new InteractiveMode(runtime, {
    initialMessage: "Please repeat alice@example.com and café.",
...
await assert.rejects(session.prompt("123-45-6789"), AdmissionError);
assert.equal(session.history.length, 0);
assert.equal(session.entries.length, 0);
await session.prompt("Please repeat alice@example.com and café.");
...
for (const snapshot of [
  ...
  assert.ok(
    !snapshot.includes("alice@example.com") && !snapshot.includes("123-45-6789"),
  );
}
