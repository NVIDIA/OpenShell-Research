...
mod tests {
    ...
    #[test]
    fn deny_wins_and_json_escaping_cannot_bypass_policy() {
        let body = json!({
            "schema_version": "openshell.pi-message.v1",
            "origin": "user",
            "text": "mail me at alice@example.com, id 123-45-6789"
        });
        assert!(matches!(
            evaluate_candidate("user_message", body),
            CandidateDecision::Deny("ssn_detected")
        ));
        let escaped: Value = serde_json::from_str(
            r#"{"schema_version":"openshell.pi-message.v1","origin":"user","text":"alice\u0040example.com"}"#,
        )
        .unwrap();
        match evaluate_candidate("user_message", escaped) {
            CandidateDecision::Replace(value) => assert_eq!(value["text"], "[EMAIL]"),
            _ => panic!("decoded email should be replaced"),
        }
    }

    #[test]
    fn executable_and_signed_content_is_not_rewritten() {
        let body = json!({
            "schema_version": "openshell.pi-assistant-message.v1",
            "text": "ok",
            "tool_calls": [{"id":"1","name":"bash","arguments":{"command":"echo alice@example.com"},"thought_signature":null}],
            "thinking": []
        });
        assert!(matches!(
            evaluate_candidate("assistant_message", body),
            CandidateDecision::Deny("immutable_content_detected")
        ));
    }

    #[test]
    fn egress_inspects_decoded_content_without_rewriting() {
