...
def _handle(job: Job, *, include_output: bool = False) -> dict[str, object]:
    result: dict[str, object] = {"providerJobId": job.id, "state": job.state}
    if job.cleanup_error:
        result["cleanupError"] = job.cleanup_error
        result["sandboxName"] = job.sandbox_name
    if job.failure_code:
...
def create_app(
    ...
    def authenticate(authorization: Annotated[str | None, Header()] = None) -> None:
        ...
        if scheme.lower() != "bearer" or not secrets.compare_digest(
            supplied.encode(), settings.token.encode()
        ):
            raise HTTPException(status_code=401, detail="invalid bearer token")

    async def find_job(job_id: str) -> Job:
        job = await asyncio.to_thread(service.store.get, job_id)
        if job is None:
