...
def test_llm_reviewer_sends_structured_untrusted_inputs() -> None:
    ...
    assert set(schema["required"]) == {"decision", "reason", "violations"}
...
def test_review_schema_rejects_invalid_fields(overrides: dict[str, object]) -> None:
    response = {"decision": "deny", "reason": "permission increase", "violations": []}
    ...
    reviewer = LlmPolicyReviewer(
        ...
        model="test-model",
        timeout_seconds=10,
        requester=lambda _payload: response_body(response),
    )
    with pytest.raises(PolicyReviewError):
        reviewer.review(request())
