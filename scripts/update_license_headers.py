...
SUPPORTED_EXTENSIONS = frozenset([".css", ".py", ".sh"])
...
def is_comment_line(line: str) -> bool:
    """Return whether a line is a supported single-line comment."""
    ...
    for i in range(start_idx, min(start_idx + MAX_HEADER_SEARCH_LINES, len(lines))):
        ...
        if re.search(r"SPDX", line, re.IGNORECASE):
            ...
            if stripped == "":
                ...
                end_idx = i + 1
            break
        elif stripped == "" or is_comment_line(stripped):
...
def get_copyright_year_string(file_path: Path, current_year: int, existing_header: str = "") -> str:
    ...
    Returns:
        ...
        - A range if file was created in a previous year (e.g., "2024-2026")
    """
...
def generate_license_header(copyright_year: str, file_path: Path) -> str:
...
def main(path: Path, check_only: bool = False) -> tuple[int, int, int, list[Path]]:
    ...
    for ext in SUPPORTED_EXTENSIONS:
        for file_path in path.glob(f"**/*{ext}"):
            ...
            license_header = generate_license_header(copyright_year, file_path)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Add or check license headers in Python, shell, and CSS files")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    if args.check:
        ...
        if all_files_needing_update:
            ...
            for file_path in all_files_needing_update:
                print(f"   • {file_path}")
            print("💡 Run 'python scripts/update_license_headers.py' to fix")
            sys.exit(1)
        else:
